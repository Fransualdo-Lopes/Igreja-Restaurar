  ...
  return (
    ...
    >
        ...
        >
          ...
          <div className="relative h-full flex items-center justify-center text-center px-4">
            <div className="max-w-4xl animate-in fade-in slide-in-from-bottom-8 duration-1000">
              <span className="inline-block py-1 px-3 border border-white/50 rounded-full text-white text-[10px] md:text-sm uppercase tracking-widest mb-6 backdrop-blur-md bg-white/10">
                {slide.badge}
              </span>
              <h1 className="text-4xl md:text-7xl lg:text-8xl font-serif text-white mb-6 leading-tight">{slide.title}</h1>
              <p className="text-gray-200 text-sm md:text-xl mb-10 max-w-2xl mx-auto">{slide.subtitle}</p>
              <div className="flex flex-col md:flex-row gap-4 justify-center">
                <button 
                  onClick={() => slide.id === 1 ? onOpenGivingModal('oferta') : window.open('https://maps.app.goo.gl/sMLBfgzwL28irDcJ8')}
                  className="bg-[#D64531] text-white px-8 py-3 md:py-4 rounded-full font-bold uppercase text-xs md:text-sm hover:scale-105 transition-transform"
                >
      ...
      <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-4 z-30 pointer-events-none">
        <button onClick={prevSlide} className="pointer-events-auto p-2 md:p-4 rounded-full bg-black/20 text-white/50 hover:text-white backdrop-blur-sm border border-white/10">
          <ChevronLeft size={20} className="md:w-8 md:h-8" />
        </button>
        <button onClick={nextSlide} className="pointer-events-auto p-2 md:p-4 rounded-full bg-black/20 text-white/50 hover:text-white backdrop-blur-sm border border-white/10">
          <ChevronRight size={20} className="md:w-8 md:h-8" />
        </button>
      </div>

      {/* Address Footer */}
      <div className="absolute bottom-6 z-30 flex items-center gap-2 text-white/80 text-[10px] md:text-xs bg-black/40 px-4 py-2 rounded-full backdrop-blur-md border border-white/10">
        <MapPin size={14} className="text-[#D64531]" />
        <span>Rua Barão de Serra Azul, 403 - Jaderlândia</span>
      </div>
    </div>
  );
};
