...
export const ChatAssistant: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      <div 
        className={`fixed bottom-0 right-0 sm:bottom-6 sm:right-6 w-full sm:w-[380px] bg-white dark:bg-gray-800 sm:rounded-2xl shadow-2xl z-50 transition-all duration-300 transform ${
          isOpen ? 'translate-y-0 opacity-100' : 'translate-y-[120%] opacity-0'
        } flex flex-col max-h-[100vh] sm:max-h-[600px] h-[100dvh] sm:h-auto border border-gray-200 dark:border-gray-700`}
      >
        {/* Header */}
        ...
        {/* Messages */}
        <div className="flex-1 overflow-y-auto p-4 bg-[#F9F7F2] dark:bg-gray-900 space-y-4 transition-colors">
        ...
        {/* Input */}
        <div className="p-4 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 transition-colors pb-safe">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-full px-4 py-2.5 text-sm focus:outline-none focus:border-[#D64531] dark:text-white"
            />
            <button
              onClick={handleSend}
              disabled={isLoading || !inputText.trim()}
              className="bg-[#D64531] text-white p-2.5 rounded-full hover:bg-[#b53a29] disabled:opacity-50 transition-colors"
            >
              <Send size={18} />
            </button>
          </div>
